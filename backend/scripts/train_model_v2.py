...
def train_and_eval_robust():
    ...
    print(f"Total dataset size: {full_df.shape[0]}")
    
    # Use a proper 80/20 split to ensure test set has representation
    ...
    # Vectorizer
    vectorizer = TfidfVectorizer(max_features=5000, stop_words='english', token_pattern=r'\b\w+\b') 
    X_train_vec = vectorizer.fit_transform(X_train)
    X_test_vec = vectorizer.transform(X_test)
    
    # Model
    clf = MultiOutputClassifier(LogisticRegression(solver='liblinear', class_weight='balanced', C=1.0, random_state=42))
    ...
    for i, label in enumerate(labels):
        ...
        # Handle cases where class might be missing in small split
